...
fn independent_contexts_use_their_own_databases() {
    ...
    let first_context =
        Context::configure(ContextOptions::default().database_path(first.join("proj.db")))
            .expect("first context");
    let second_context =
        Context::configure(ContextOptions::default().database_path(second.join("proj.db")))
...
fn missing_explicit_database_is_rejected() {
    ...
    let error = match Context::configure(ContextOptions::default().database_path(missing)) {
